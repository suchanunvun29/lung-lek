...
resolveViewerScope(user):
  ...
  ถ้า user.role = SALESPERSON
     ...
     → {
         ...
         supervisedTerritoryIds: [ A ที่ isSupervisor = true ].territoryId,
...
| **SALESPERSON ที่มี `isSupervisor = true` ในเขต T** | ตัวเอง **+ ทุกคนที่มี assignment มีผลอยู่ในเขต T** | ทุกคนที่มี assignment มีผลอยู่ในเขต T (**รวมตัวเขาเอง** — เขาเป็นสมาชิกของทีมนั้นจริง · ยืนยัน 2026-08-19 การตัดสินใจแถวที่ 28) |
...
**สิทธิ์**: MANAGER เท่านั้นที่จัดการบัญชีได้ · ~~ทุก role เห็นข้อมูลธุรกิจเท่ากันหมด~~ **เปลี่ยนแล้ว 2026-08-19 — เห็นไม่เท่ากันแล้ว** ดูหัวข้อสัญญา **Data Visibility Rules** และ Module Q
**ปรับ 2026-08-19**: **schema ของ module นี้ไม่เปลี่ยนเลย** (`User`/`Salesperson` คงเดิมทุกตัวอักษร) · สิ่งที่เพิ่มคือ **งานข้อมูล** — ต้องผูก `Salesperson.userId` ให้ครบทุกบัญชี `SALESPERSON` และหน้าจัดการผู้ใช้ต้องขึ้นเตือนบัญชีที่ยังไม่ผูก (Data Visibility Rules ข้อ 5) เพราะหลังบังคับ 3 ชั้น บัญชีที่ไม่ได้ผูกจะไม่มีข้อมูลของตัวเองให้ดูเลย
...
เพิ่ม 2026-08-19 · ครอบคลุม `requirement.md` OQ25 · OQ27 · OQ29 · **อ่าน Data Visibility Rules ทั้งหัวข้อก่อน implement**
- `resolveViewerScope(user)` **จุดเดียวของทั้งระบบ** + middleware/helper ที่ทุก service เรียกใช้ · ห้ามกระจาย `if (role === 'MANAGER')` ไปทั่ว
- ใส่การกรองเข้าไปในทุก endpoint ของ Phase 4–7 ที่ส่งข้อมูลรายคน (KPI รายคน, dashboard, ตัวสลับดูคนอื่น, insight ทั้งอ่านและสั่งสร้าง, รายงานรายบุคคล, ภาพรวมทีม, ทุก Export)
- กรณีบัญชีที่ยังไม่ผูก `Salesperson` (Data Visibility Rules ข้อ 5) + คำเตือนในหน้าจัดการผู้ใช้ของ Module B
- ปรับสิทธิ์สั่งสร้าง AI insight เป็น MANAGER + supervisor (เปลี่ยนรอบที่ 2 ของ Phase 6)
...
- **role ที่ 3 (`SALES_SUPERVISOR`) ใน `enum UserRole`** — ผู้ใช้ยืนยัน 2026-08-19 ว่า supervisor เป็น **flag บนคู่ (คน × เขต)** ไม่ใช่คุณสมบัติของตัวคน · ระบบยังมี 2 role เท่าเดิม ห้ามเพิ่มค่าใน `UserRole`
