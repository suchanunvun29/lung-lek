...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = "default", size = "default", ...props }, ref) => {
    const variantStyles: Record<NonNullable<ButtonProps["variant"]>, string> = {
      ...
      success: "bg-success text-white hover:bg-success-hover shadow-xs",
    };

    /* One touch-target idiom for every size (T-UX-019): the token floor (44px)
       applies on touch-sized screens; desktop shrinks via explicit lg: sizes. */
    const sizeStyles: Record<NonNullable<ButtonProps["size"]>, string> = {
      default: "h-9 min-h-(--touch-target) lg:min-h-9 px-4 py-2 text-sm",
      sm: "h-8 px-3 text-xs rounded-md min-h-(--touch-target) lg:min-h-8",
      lg: "h-11 px-8 text-base rounded-md min-h-(--touch-target)",
      icon: "h-9 w-9 p-0 min-h-(--touch-target) min-w-(--touch-target) lg:min-h-9 lg:min-w-9",
    };
