...
export default function PeriodPicker({ value, onChange, disabled }: PeriodPickerProps) {
      ...
      <div className="flex flex-wrap items-end gap-3">
        <label className="grid gap-1 text-sm font-medium text-zinc-700">
          ...
          <input
            ...
            disabled={disabled}
            className="w-28 rounded-md border border-zinc-300 px-3 py-2 text-sm disabled:bg-zinc-100"
          />
        </label>
        <label className="grid gap-1 text-sm font-medium text-zinc-700">
          เดือน
        ...
        {value.map((period) => (
          <span key={`${period.year}-${period.month}`} className="inline-flex items-center gap-2 rounded-full bg-zinc-100 px-3 py-1 text-sm text-zinc-700">
            ...
            <button
              ...
              className="font-semibold text-zinc-500 hover:text-zinc-900 disabled:cursor-not-allowed"
            >
              ×
            </button>
          </span>
