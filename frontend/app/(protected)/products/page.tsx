...
export default function ProductsPage() {
  ...
  async function handleSave(
    ...
    try {
      ...
      setError(getErrorMessage(saveError, "บันทึกสินค้าไม่สำเร็จ"));
    }
  }

  return (
    <div className="mx-auto max-w-6xl p-4 sm:p-6">
      <h1 className="text-2xl font-semibold text-zinc-900">ทะเบียนสินค้า</h1>
      <p className="mt-1 text-sm text-zinc-600">รายการที่สร้างจากประวัติการขายจะยังไม่มีรหัสสินค้า</p>
      {!canEdit && <p className="mt-1 text-sm text-zinc-600">คุณสามารถดูข้อมูลได้เท่านั้น การแก้ไขสงวนไว้สำหรับผู้จัดการ</p>}
      {loading && <p className="mt-6 text-zinc-400">กำลังโหลด...</p>}
      {error && <p className="mt-4 text-sm text-red-600">{error}</p>}
      {!loading && (
        <div className="mt-4">
          <ProductMasterTable products={products} canEdit={canEdit} onSave={handleSave} />
        </div>
      )}
    </div>
  );
