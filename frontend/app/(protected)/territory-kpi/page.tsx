...
export default function TerritoryKpiPage() {
  ...
  async function exportReport() {
    ...
    try {
      ...
      setError(getErrorMessage(exportError, "ส่งออกรายงานไม่สำเร็จ"));
    }
  }
  ...
  return (
    <div className="mx-auto max-w-7xl p-4 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-zinc-900">รายงาน KPI รายเขต</h1>
          <p className="mt-1 text-sm text-zinc-600">เปรียบเทียบยอดขาย เป้า และคะแนนรวมของแต่ละเขต</p>
        </div>
      ...
      {data && !loading && (
        <div className="mt-6 space-y-5">
          <TerritoryKpiTable territories={data.territories} onDrillDown={(territory, metric) => setDrillDown({ territory, metric })} />
          <TerritoryGroupKpiTable groups={data.territoryGroups} />
          {data.buckets && (
            <section className="rounded-lg border border-zinc-200 bg-white p-4">
              <h2 className="text-base font-semibold text-zinc-900">ยอดนอกเขต</h2>
              <p className="mt-1 text-sm text-zinc-600">ยอดบริษัท = ผลรวมของทุกเขต + ยอดส่วนบุคคล + ยอดที่ยังไม่กำหนดเขต</p>
              <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4">
                <div>
                  <dt className="text-zinc-500">ยอดบริษัท</dt>
                  <dd className="mt-1 font-semibold text-zinc-900">{formatMoney(data.buckets.companyTotal)}</dd>
                </div>
                <div>
                  <dt className="text-zinc-500">ยอดรวมทุกเขต</dt>
              ...
              {data.buckets.personalBucketEntries.length > 0 && (
                <table className="mt-4 min-w-full divide-y divide-zinc-200 rounded-lg border border-zinc-200 text-sm">
                  <thead className="bg-zinc-50 text-left text-xs font-medium uppercase tracking-wide text-zinc-500">
                    <tr>
                      <th className="px-4 py-3">ผู้ใช้เป้าส่วนบุคคล</th>
