...
1. Read `requirement.md` and `design.md` in the module folder — specifically the roles/permissions in `requirement.md` and any module flagged as handling a sensitive concern in `design.md`. You can't judge an authorization bug without knowing who was supposed to have access.
2. Read `review.md` if it exists, so you know what `qa-engineer` already found and don't re-report the same functional gaps as security issues.
